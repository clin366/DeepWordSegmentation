...
tf.app.flags.DEFINE_integer("train_steps", 10000, "trainning steps")
...
def main(unused_argv):
    ...
    with graph.as_default():
        ...
        with sv.managed_session(master = '') as sess:
            sv.saver.restore(sess, FLAGS.log_dir + '/final-model')
            print ("Start Restore ================================")
            _, trainsMatrix = sess.run([train_op, model.transition_params])
            print ("Start Test !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! ")
            test_evaluate(sess, test_unary_score, test_sequence_length, trainsMatrix, model.inp_w, model.inp_c, twX, tcX, tY)
